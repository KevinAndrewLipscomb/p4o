...
namespace UserControl_role_privilege_matrix
{
    ...
    public partial class TWebUserControl_role_privilege_matrix: ki_web_ui.usercontrol_class
    {
            ...
            if (row.Cells.Count > Class_db_role_privilege_map.Units.Class_db_role_privilege_map.CI_FIRST_CROSSTAB)
            {
                for (i = Class_db_role_privilege_map.Units.Class_db_role_privilege_map.CI_FIRST_CROSSTAB; i <= (row.Cells.Count - 1); i ++ )
                {
                    if (row.RowType == DataControlRowType.DataRow)
                    {
                        ...
                        check_box.ID = kix.Units.kix.EMPTY + Units.UserControl_role_privilege_matrix.CHECKBOX_ID_PREFIX_PRIVILEGE_ID + row.Cells[Class_db_role_privilege_map.Units.Class_db_role_privilege_map.CI_PRIVILEGE_ID].Text + Units.UserControl_role_privilege_matrix.CHECKBOX_ID_PREFIX_ROLE_ID + crosstab_metadata_rec.id;
        ...
        private void Changed(object sender, System.EventArgs e)
        {
            ...
            p.biz_role_privilege_map.Save(kix.Units.kix.Safe(tuple[TUPLE_INDEX_PRIVILEGE_ID], kix.safe_hint_type.NUM), kix.Units.kix.Safe(tuple[TUPLE_INDEX_ROLE_ID], kix.safe_hint_type.NUM), check_box.Checked);
        }
