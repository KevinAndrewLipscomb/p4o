...
type
  TClass_db_bureaus = class(TClass_db)
  published
    function Bind
      (
      partial_spec: string;
...
constructor TClass_db_bureaus.Create;
begin
  inherited Create;
...
function TClass_db_bureaus.Bind
  (
  partial_spec: string;
  target: system.object
  )
...
begin
  ...
  //
  self.Open;
  dr := mysqlcommand.Create('SELECT id,description FROM bureau where description <> "(none specified)" order by id',connection).ExecuteReader;
  while dr.Read do begin
...
function TClass_db_bureaus.Delete(id: string): boolean;
begin
  //
  Delete := TRUE;
  //
  self.Open;
  try
    mysqlcommand.Create(db_trail.Saved('delete from bureau where id = ' + id),connection).ExecuteNonquery;
...
function TClass_db_bureaus.Get
  (
  id: string;
  out description: string
  )
...
begin
  Get := FALSE;
  self.Open;
  dr := mysqlcommand.Create('select description from bureau where id = "' + id + '"',connection).ExecuteReader;
...
procedure TClass_db_bureaus.&Set
  (
  id: string;
  description: string
  );
