...
namespace Class_db_role_privilege_map
{
    public class TClass_db_role_privilege_map: TClass_db
    {
        ...
        public void BindActuals(string sort_order, bool be_sort_order_ascending, object target)
        {
            ...
            this.Open();
            ((target) as GridView).DataSource = new MySqlCommand("select role_id" + " , pecking_order as role_pecking_order" + " , role.name as role_name" + " , privilege.name as privilege_name" + " , privilege_id" + " from role_privilege_map" + " join privilege on (privilege.id=role_privilege_map.privilege_id)" + " join role on (role.id=role_privilege_map.role_id)" + " order by " + sort_order, this.connection).ExecuteReader();
            ((target) as GridView).DataBind();
            this.Close();
